...
def _remote_producer(init):
    """
    Produce a remote input function by loading a :class:`snakemake.remote`
    module and passing the module to an *init* function.

    The module is named by the first argument of the *init* function.  *init*
    will be called with the loaded module and is expected to return a remote
    input function suitable for use inside :func:`path_or_url`.
    ...
    remote_name = init.__code__.co_varnames[0]
    remote = None
